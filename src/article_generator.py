...
**パターン1: 数字入り（項目数が明確な場合のみ）**
- 【2026年版】〜初心者が絶対にハマるN個の落とし穴｜後悔しないための〜
- 【保存版】〜で差がつくN個のテクニック｜プロが教える実践的手順
- 〜できない原因はコレ！即効で改善するN個のチェックポイント
- **重要：数字を入れる場合、記事本文で必ずその数だけ説明すること**

**パターン2: ノウハウ・ガイド系（数字なし）**
- 【完全ガイド】〜を劇的に改善する方法｜初心者でもできる実践テクニック
- 【保存版】〜の正しい使い方｜プロが実践する〜術
- 【徹底解説】〜で失敗しないための基礎知識｜今すぐ実践できる〜

**パターン3: 問題解決系（数字なし）**
- 〜できない原因はコレ！即効で解決する実践的アプローチ
- 〜が〜しない理由とは？プロが教える本質的な解決策
- 〜の悩みを解消｜知らないと損する〜のメカニズム

**パターン4: 比較・選び方系（数字なし）**
- 〜と〜の違いとは？初心者でも分かる使い分け完全ガイド
- 【徹底比較】〜選びで後悔しないための判断基準｜2026年最新版
- 〜を選ぶ前に知っておきたい本質的な違い｜プロが教える〜

**パターン5: メカニズム・本質系（数字なし）**
- 〜の本質を理解する｜初心者が見落としがちな〜のメカニズム
- なぜ〜は〜なのか？プロが解説する音響学的アプローチ
- 〜の正体とは｜知っておくべき〜の基礎知識

## タイトル作成のルール
1. **パターンをバランスよく使う**：パターン1〜5をランダムに選び、タイトルに多様性を持たせる
2. **数字入りの判断基準**：記事内容が「〜の3つの方法」「5つのポイント」など項目列挙型なら数字入り、それ以外は数字なし
3. **数字のバリエーション**：数字入りタイトルを使う場合、**2〜7の範囲でバラバラな数字を使う**
   - 良い例：「3つの方法」「4つのポイント」「6つの秘訣」「7つのステップ」
   - 悪い例：いつも「5つの〜」（機械的に見える）
   - **記事ごとに異なる数字を使い、同じ数字が続かないようにする**
4. **数字を使う場合の厳守事項**：タイトルの数字と本文の項目数は必ず一致させること
5. ターゲット明確（初心者、DTMer、宅録派など）
6. ベネフィット明示（後悔しない、差がつく、劇的に改善など）
7. 強い言葉を使う（絶対に、即効、完全、徹底など）
8. 30〜40文字程度

**重要：すべてのパターンをバランスよく使い、数字を使う場合は2〜7でバラけさせること！**
...
[CONTENT]
HTML形式の記事本文（以下のタグを使用）：
- <h2>, <h3>（見出し）
- <p>（段落）
- <strong>（重要なキーワード・専門用語の強調）
- <mark>（黄色マーカー：段落の結論や最重要ポイントの強調）
- <ul>, <li>（箇条書き）
- <table>, <thead>, <tbody>, <tr>, <th>, <td>（表）
[/CONTENT]

# 読みやすさの重要ルール
1. **段落は2-3文で短く**: 1段落は2-3文程度。長い説明は複数の<p>タグに分割すること
2. **余白は入れない**: <p>&nbsp;</p>は一切使用しない。WordPressのデフォルトmarginに任せる
3. **太字を積極的に使う**: 各段落で重要なキーワード・専門用語は必ず<strong>で太字にする
   - 1段落に1-2個は太字を入れる
   - 例：「<strong>ベロシティレイヤー</strong>」「<strong>位相の問題</strong>」
4. **黄色マーカーで核心を強調**: 段落の結論や最重要ポイントには<mark>タグで黄色マーカーを使う
   - 使用頻度：記事全体で3-5箇所程度（控えめに）
   - 例：「<mark>ボーカルが埋もれる最大の原因は周波数の競合です</mark>」
   - 段落の要約や結論部分に使う
   - **重要：表（<table>）の中では絶対に使用しない**
5. **箇条書きを活用**: 手順や要点は<ul><li>で明確に
6. **具体例は別段落**: 「例えば〜」で始まる具体例は必ず新しい<p>タグで
7. **小見出しで区切る**: 長いセクションは<h3>で細分化
8. **1文は短めに**: 接続詞で繋げすぎず、適度に文を区切る
9. **表を積極的に使う**: 以下のような内容は<table>タグで表にまとめると視覚的でわかりやすい
   - 比較（機材、ソフト、手法など）
   - 設定値一覧（周波数、ゲイン、タイムなど）
   - チェックリスト（問題と解決策）
   - ステップバイステップの手順
...
良い例（2-3文＋太字＋マーカー）：
<p>同じ「<strong>スネア</strong>」でも、叩く強さや場所によって<mark>音が全然違います</mark>よね。</p>

<p><strong>リムショット</strong>（フチを叩く）、<strong>オープンリムショット</strong>、<strong>ゴーストノート</strong>（弱く叩く）など、様々な音色があります。打ち込みでも、これらの<strong>音色の違い</strong>を表現することが大切です。</p>

<h3>ベロシティレイヤーを活用しましょう</h3>

<p>ほとんどのドラム音源には、<strong>ベロシティの強弱</strong>に応じて異なるサンプル（実際に録音された音）が鳴る仕組みがあります。</p>

<p>これを<mark>「ベロシティレイヤー」</mark>と呼びます。この機能を使うことで、リアルな演奏表現が可能になるんです。</p>

# 表（table）の使用例

比較や設定値は表で示すと格段にわかりやすくなります：
...
<p>このように、<strong>表を使うことで情報が整理</strong>されて比較しやすくなります。</p>
...
<p><mark>数値を表で示すことで、設定の全体像が把握しやすくなります</mark>。</p>
